#include <ESP8266WiFi.h>;
#include <WiFiClientSecure.h>
#include <ArduinoJson.h>
#include <time.h>
#include <PubSubClient.h>;
#include <NTPClient.h>;
#include <WiFiUdp.h>;
#include <AESLib.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
#include "FS.h"

AESLib aesLib;
byte Akey[] = {0x4E, 0x5A, 0x1B, 0xE2, 0x78, 0xDE, 0x08, 0x9A, 0x89, 0x0D, 0x6E, 0x9C, 0x05, 0x85, 0x99, 0x90}; // 128-bit key


#define CLIENT_ID         "kitchen"
#define CLIENT_USERNAME   "kitchen"
#define CLIENT_PSW        "k1tCK3n_STR0ng"
#define SUB_TOPIC         "iot/rooms/kitchen"
#define TIME_ZONE -5
#define THINGNAME "ESP8266"
#define AWS_IOT_PUBLISH_TOPIC   "esp8266/pub"
#define AWS_IOT_SUBSCRIBE_TOPIC "esp8266/sub"

/* Set the value for the Access Point */
const char* ssid = "GioNet";
const char* password = "shish9999";
const char MQTT_HOST[] = "ad6k0u45kocwb-ats.iot.us-east-1.amazonaws.com";
uint8_t ivAES[16] = {0x8C, 0x0A, 0x15, 0x8E, 0xD8, 0x36, 0xC8, 0x24, 0xB6, 0x06, 0x7A, 0x95, 0x4F, 0xF3, 0x06, 0x4F};

const int RX_PIN = 3; // GPIO3 (RX) of ESP8266 connected to STM32 TX
const int TX_PIN = 1; // GPIO1 (TX) of ESP8266 connected to STM32 RX


static const char cacert[] PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----

rqXRfboQnoZsG4q5WTP468SQvvG5
-----END CERTIFICATE-----
)EOF";
 
 
// Copy contents from XXXXXXXX-certificate.pem.crt here
static const char client_cert[] PROGMEM = R"KEY(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
 
)KEY";
 
 
// Copy contents from  XXXXXXXX-private.pem.key here
static const char privkey[] PROGMEM = R"KEY(
-----BEGIN RSA PRIVATE KEY-----

c9BtprjxG8ToOIsK3W8Up65Jwzl5VAVbMb76uN+0KotOQsG+oynU
-----END RSA PRIVATE KEY-----
 
)KEY";

/* Timing libraries */
WiFiUDP ntpUDP;
NTPClient timeClient(ntpUDP, "pool.ntp.org");
WiFiClientSecure net;
time_t now;
time_t nowish = 1510592825;
 
BearSSL::X509List cert(cacert);
BearSSL::X509List client_crt(client_cert);
BearSSL::PrivateKey key(privkey);
 
PubSubClient client(net);

void messageReceived(char *topic, byte *payload, unsigned int length)
{
  Serial.print("Received [");
  Serial.print(topic);
  Serial.print("]: ");
  for (int i = 0; i < length; i++)
  {
    Serial.print((char)payload[i]);
  }
  Serial.println();
}

void publishMessage(const byte *password) {
  // Create JSON document
  StaticJsonDocument<300> doc;
  doc["password"] = password;

  // Serialize JSON document to char buffer
  char jsonBuffer[512];
  serializeJson(doc, jsonBuffer);

  // Publish JSON message to MQTT broker
  client.publish(AWS_IOT_PUBLISH_TOPIC, jsonBuffer);
}

void NTPConnect(void)
{
  Serial.print("Setting time using SNTP");
  configTime(TIME_ZONE * 3600, 0 * 3600, "pool.ntp.org", "time.nist.gov");
  now = time(nullptr);
  while (now < nowish)
  {
    delay(500);
    Serial.print(".");
    now = time(nullptr);
  }
  Serial.println("done!");
  struct tm timeinfo;
  gmtime_r(&now, &timeinfo);
  Serial.print("Current time: ");
  Serial.print(asctime(&timeinfo));
}

void setup_wifi() {
  delay(10);
  
  /* Connect to WiFi network and set the buffer size for the communication */
  net.setBufferSizes(1024, 1024);
  Serial.println();
  Serial.print("Connecting to ");
  Serial.println(ssid);
  WiFi.begin(ssid, password);

  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
  }
  Serial.print("WiFi connected\n");
  Serial.print("IP address: ");
  Serial.print(WiFi.localIP());
  Serial.print("\n");
}

void connectAWS()
{
  delay(3000);
  setup_wifi();
 
  NTPConnect(); //funzione timezone
 
  net.setTrustAnchors(&cert);
  net.setClientRSACert(&client_crt, &key);

  Serial.println("Set the anchors");
 
  client.setServer(MQTT_HOST, 8883);
  client.setCallback(messageReceived);
 
  Serial.println("Connecting to AWS IOT");
 
  while (!client.connect(THINGNAME))
  {
    Serial.print(".");
    delay(1000);
  }
 
  if (!client.connected()) {
    Serial.println("AWS IoT Timeout!");
    return;
  }
  // Subscribe to a topic
  client.subscribe(AWS_IOT_SUBSCRIBE_TOPIC);
 
  Serial.println("AWS IoT Connected!");
}

bool isByteArrayEmpty(unsigned char arr[], size_t size) {
  for (size_t i = 0; i < size; i++) {
    if (arr[i] != 0) {
      return false; // If any element is non-zero, the array is not empty
    }
  }
  return true; // All elements are zero, array is empty
}

void setup() {
  Serial.begin(115200);
  //Serial.setRX(RX_PIN); // Set RX pin
  //Serial.setTX(TX_PIN); // Set TX pin
  delay(1000);
  connectAWS();
}

void loop() {
  byte encrypted_data[16];
  byte decrypted_data[16];
  
  if (Serial.available()) {
    Serial.readBytes(encrypted_data, sizeof(encrypted_data));
    //aesLib.do_aes_decrypt(encrypted_data, 16, decrypted_data, key, 128);
    uint16_t dec_bytes = aesLib.decrypt(encrypted_data, 16,  decrypted_data, Akey, 128, ivAES);

    Serial.println("Decrypted data:");
    //Serial.write(decrypted_data, sizeof(decrypted_data));
  }
  

  if (!client.connected()){
    connectAWS();
    Serial.print("WiFi connected\n");
  } else {
    client.loop();
    delay(5000);
    publishMessage(decrypted_data);
  }
}
